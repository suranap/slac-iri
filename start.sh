...
# Verify the Slurm service is running. Get the SLURM_JWT token.
export $(docker compose --file slurm/docker-compose.yml exec slurmrestd scontrol token)
curl -k -H X-SLURM-USER-TOKEN:${SLURM_JWT} -H X-SLURM-USER-NAME:root -X GET 'http://localhost:9200/slurm/v0.0.41/diag' 

# Start the PSI/J service
