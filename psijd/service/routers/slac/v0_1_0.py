...
def get_slac_executor_with_token(access_token: str = Depends(get_bearer_token)):
    """Get SLAC executor for this version, configured with the access token."""
...
async def submit_job(
    job_request: JobEx,
    executor=Depends(get_slac_executor_with_token)
):
    """Submit a new job to SLAC (v0.1.0)"""
    logger.info(f"Received job request for {BACKEND} {VERSION}: {job_request}")
    try:
        # Convert JobEx to JobSpec
        ...
        logger.info(f"Job submitted: {job._native_id}")
...
async def health_check():
    """Health check for the SLAC backend (v0.1.0)"""
